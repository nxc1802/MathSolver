...
(1) User Input: Text toán học hoặc ảnh đề bài
    │
    ▼
(2) OCR Agent (khi có ảnh: upload OCR, image_url trong solve, hoặc sau bước ocr_preview):
    ├─ IN: file ảnh (jpg, png, …)
...
- **Solve:** `POST /api/v1/sessions/{session_id}/solve` — JWT, JSON `text`, `image_url` tùy chọn — trả `job_id`, `status: processing`.
