  ...
  return (
    ...
    >
      ...
      <div className="flex-1 h-full flex flex-col bg-[#08080d] overflow-hidden">
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-8 scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent">
          <AnimatePresence mode="popLayout">
            {/* 1. Static Geometry (Always first if available) */}
            {coordinates && (
              <motion.div
                key="canvas-section"
              ...
              >
                <div className="mb-3 flex items-center justify-between px-1">
                  <h3 className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">Hình vẽ kỹ thuật</h3>
                </div>
                <div className="bg-[#0c0c14] rounded-2xl border border-white/5 p-4 shadow-2xl">
                   <StaticGeometryCanvas coordinates={coordinates} />
                </div>
              </motion.div>
            )}

            {/* 2. Video Animation (If requested/available) */}
              ...
              >
                <div className="mb-3 flex items-center justify-between px-1">
                  <h3 className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">
                    {videoUrl ? "🎬 Phim minh họa" : "🎨 Đang tạo animation..."}
                  </h3>
                </div>
                <div className="bg-[#0c0c14] rounded-2xl border border-white/5 p-4 shadow-2xl">
            ...
            {!coordinates && !videoUrl && !renderingVideo && (
              ...
              >
                <div className="w-20 h-20 mx-auto rounded-3xl bg-white/[0.02] border border-white/5 flex items-center justify-center">
                  <svg className="w-10 h-10 text-zinc-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
                    <path d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                  </svg>
                </div>
                <div>
                  <p className="text-zinc-500 text-sm font-medium">Bảng trực quan</p>
                  <p className="text-zinc-700 text-xs mt-1">Kết quả hình học sẽ xuất hiện tại đây</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
