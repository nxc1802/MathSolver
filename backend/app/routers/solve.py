...
from app.jobs.solve_session_job import run_solve_session_job as process_session_job
...
def _enqueue_solve_common(
    ...
    """Insert user message, job row, enqueue solve pipeline on Celery queue `solve`."""
    ...
    q = os.getenv("CELERY_SOLVE_QUEUE", "solve")
    process_solve_session_job.apply_async(
        args=[job_id, session_id, str(user_id), request.text, request.image_url],
        queue=q,
    )
